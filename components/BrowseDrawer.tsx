...
import { CoverrVideo, HydratedCategory } from '@/lib/types';
import { fetchCurated } from '@/lib/coverr';
...
const SAVED_TAB = { id: 'favorites', name: 'Saved', emoji: '♥' };

export default function BrowseDrawer({ open, favorites, onClose, onSelect, onFavorite }: Props) {
  const [tab, setTab] = useState(() => storage.getCategory() ?? 'favorites');
  ...
  const currentVideos = tab === 'favorites'
    ? favorites
    : (categories.find(c => c.id === tab)?.videos ?? []);

  const allTabs = [...categories.map(c => ({ id: c.id, name: c.name, emoji: c.emoji })), SAVED_TAB];
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <button onClick={onClose} className="text-white/60 hover:text-white text-2xl leading-none transition" aria-label="Close">×</button>
        </div>
                ...
                {allTabs.map(t => (
                  <button
                    ...
                    className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-medium transition ${
                      tab === t.id ? 'bg-white text-black' : 'bg-white/10 text-white hover:bg-white/20'
                    }`}
                  >
                    {t.emoji} {t.name}
                  </button>
            ...
            <div className="overflow-y-auto h-[calc(100%-8rem)] px-4 pb-4">
              {currentVideos.length === 0 ? (
                <div className="flex items-center justify-center h-32 text-white/40 text-sm">
                  {tab === 'favorites' ? 'No saved scenes yet' : 'No scenes in this category'}
                </div>
              ) : (
