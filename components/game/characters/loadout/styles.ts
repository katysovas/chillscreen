...
export const LOADOUT_STYLES = `
  ...
  .ch-right-hand .ch-lo-brush{position:absolute;left:-2px;top:-118px;width:120px;height:120px;transform-origin:50% 88%;z-index:12;pointer-events:none;}
  /* Cancel hand tilt; +40° extra when mirrored (facing left). */
  .ch-painting-brush:not(.ch-walking) .ch-lo-brush{transform:rotate(calc(47deg * var(--ch-mirror,1) + 40deg * (1 - var(--ch-mirror,1)) / 2));}
  ...
  /* Easel painting — arm reach + brush stroke (incl. Deep Space float). */
  .ch-painting-brush:not(.ch-walking) .ch-right-hand{
    animation:ch-painting-hand 1.6s ease-in-out infinite!important;
    top:70px!important;
  }
  @keyframes ch-painting-hand{
  ...
  .ch-right-hand .ch-lo-coffee{position:absolute;left:-99px;top:-22px;width:220px;height:138px;transform:rotate(40deg) scale(0.82);transform-origin:50% 88%;z-index:12;pointer-events:none;}
  .ch-lo-coffee-img{display:block;width:100%;height:100%;object-fit:contain;object-position:center bottom;pointer-events:none;user-select:none;}
